...
async function getQuotationWasteByClient(req, res) {
  try {
    ...
    const quotations = await Quotation.findAll({
      where: { clientId },
      include: [{ model: QuotationWaste, as: "QuotationWaste" }],
    });
...
async function getQuotationTransportationByClient(req, res) {
  try {
    ...
    const transportation = quotations.flatMap(
      (quotation) => quotation.QuotationTransportation
    );
  ...
  try {
    const vehicleLogs = await VehicleLog.findAll({
      attributes: [
        "plateNumber",
        [
          sequelize.fn("max", sequelize.col("VehicleLog.createdAt")),
          "latestCreatedAt",
        ],
      ],
      include: [
      ...
      group: ["plateNumber"],
      order: [
        [sequelize.fn("max", sequelize.col("VehicleLog.createdAt")), "DESC"],
      ],
    });
...
async function getMarketingTransactionsByMonth(req, res) {
  try {
    ...
    const result = {
      pending: [
        {
          month: prevMonth2Start.getMonth() + 1,
          year: prevMonth2Start.getFullYear(),
          count: pendingCounts[0],
        },
        {
          month: prevMonth1Start.getMonth() + 1,
          year: prevMonth1Start.getFullYear(),
          count: pendingCounts[1],
        },
        {
          month: currentMonthStart.getMonth() + 1,
          year: currentMonthStart.getFullYear(),
          count: pendingCounts[2],
        },
      ],
    };
    ...
    const logisticsTransaction = await LogisticsTransaction.findAll({
      include: [
        ...
        },
        {
          ...
          where: {
            logisticsTransactionId: Sequelize.col("LogisticsTransaction.id"),
          },
