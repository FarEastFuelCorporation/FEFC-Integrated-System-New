...
const SectionModal = ({
  ...
  const [treatmentProcesses, setQuotationWaste] = useState([]);
    ...
    if (open) {
      const fetchData = async () => {
        try {
          ...
          const [clientsResponse, quotationWasteResponse] = await Promise.all([
            axios.get(`${apiUrl}/api/client`),
            axios.get(`${apiUrl}/api/quotation/waste`),
          ]);

          setClients(clientsResponse.data.clients);
          setQuotationWaste(quotationWasteResponse.data.clients);
        } catch (error) {
